...
class ColumnPolicy
{
    ...
    {
        $isOwner = $column->board->project->owner_id === $user->id;
        $isMember = $column->board->project->members->contains('id', $user->id);
    ...
    {
        $isOwner = $column->board->project->owner_id === $user->id;
        $isAdmin = $column->board->project->members()->wherePivot('role', 'admin')->exists();
